...
class UploadHandler
{
    ...
    function get_file_object($file_name) {
	        ...
	        $q = "SELECT file_name,  OCTET_LENGTH(file) as size FROM $table where id = $file_name";
            ...
            $file->size = $this->formatBytes($row['size']);
            $file->url = "img/view.php?cat=$table&file=$file_name";
            $file->thumbnailUrl = "img/view.php?cat=$table&file=$file_name&thumb";
            $file->deleteUrl = "server/php/?cat=$table&id=$file_name";
            $file->id = $file_name;
            /*
...
function get_file_objects() {
        ...
        global $constraint;
        global $link;
        $q = "SELECT id FROM $table WHERE $constraint = $id";
        $res = mysqli_query($link,$q);
        if (!$res)
        {	print "Can not run $q";
        }
    ...
    function handle_image_file($file_path, $file) {
    ...
    function handle_file_upload($uploaded_file, $name, $size, $type, $error,
        ...
        //$file->upload_to_db = 
        $this->add_img($data, $size, $name);
			global $file_id;
			global $table;
			$id = $file_id;
			
            $q = "SELECT file_name,  OCTET_LENGTH(file) as size FROM $table where id = $id";
            global $link;
	        $res = mysqli_query($link,$q);
	        if (!$res)
	        {	print "Can not run $q";
            ...
            $file->name = $row['file_name'];
            $file->size = $this->formatBytes($row['size']);
            $file->url = "img/view.php?cat=$table&file=$id";
            $file->thumbnailUrl = "img/view.php?cat=$table&file=$id&thumb";
            $file->deleteUrl = "server/php/?cat=$table&id=$id";
            $file->id = $id;
	...
	{  
	    ...
	    $q ="INSERT INTO $table
                (file_type ,file, file_size, file_name, $constraint)
                VALUES
                ('{$size['mime']}', '{$data}', '{$size[3]}', '{$name}', '{$id}')";  
		global $link;
        $res = mysqli_query($link,$q);
        if (!$res)
        {	print "Can not run $q";
